...
grav_constant = 1.3e20  # [meters^3 / sec^2 / Msun]
speed_of_light = 3e8  # [meters / sec]

au_per_meter = 1 / 1.5e11  # [ au / m ]
au_per_kilometer = au_per_meter * 1e3  # [ au / km ]

grav_constant_au_days = (
    grav_constant * (au_per_meter)**3 * (seconds_per_day)**2
)  # AU^3 / day^2 / Msun
...
def calc_relative_semimajoraxis(beta, u_vec):
    """ final semi-major axis over initial semi-major axis """
    numer = beta
...
def calc_orbital_decay_time(a, m1, m2):
    """ From Peters (1964), Eq. (5.10) -- time for orbit to decay """
...
def calc_beta(total_mass_initial, total_mass_final):
    return total_mass_final / total_mass_initial
