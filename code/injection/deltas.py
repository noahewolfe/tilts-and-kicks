...
def get_data(key, nobs):
    detectable = h5ify.load(
        '../../data/vt/xp/sigma-spin-1e-3-more/detectable.hdf5'
    )
...
def taper(v):
    """ taper but actually its a hard cut! """
    return jnp.nan_to_num(-1e10 * (v >= maximum_variance), nan=0)


if __name__ == '__main__':
    posteriors, injections = get_data(jax.random.key(42), nobs)
    ...
    priors['log_sigma_spin'].minimum = np.log(1e-3 * 0.5).item()
    priors['log_sigma_spin'].maximum = np.log(1e-3 * 1.5).item()
    ...
    outdir = '../../data/runs/vt-xp-delta-test-narrow-lamb'
